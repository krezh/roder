...
pub(crate) fn conditions(v: &Value) -> Vec<Cond> {
    v.get("status")
        ...
        .map(|arr| {
            arr.iter()
                .map(|c| Cond {
                    type_: c.get("type").and_then(|x| x.as_str()).unwrap_or("").to_string(),
                    status: c.get("status").and_then(|x| x.as_str()).unwrap_or("").to_string(),
                    reason: c.get("reason").and_then(|x| x.as_str()).unwrap_or("").to_string(),
                    message: c.get("message").and_then(|x| x.as_str()).unwrap_or("").to_string(),
                })
                .collect()
        })
        .unwrap_or_default()
...
pub(crate) fn rbac_rules(o: &Value) -> Vec<PolicyRule> {
    ...
    arr.iter()
        .map(|rule| {
            ...
            let names = if urls.is_empty() { join("resourceNames") } else { urls };
            let resources = if resources.is_empty() && !names.is_empty() {
                "(non-resource)".to_string()
            } else {
                resources
            };
            PolicyRule { groups, resources, verbs: join("verbs"), names }
        })
        .collect()
}
