...
pub(crate) fn LogsView(
    ...
    #[prop(optional)] target: Option<LogTarget>,
) -> impl IntoView {
    let log_pods = expect_context::<LogPods>().0;
    ...
    Effect::new(move |_prev: Option<Option<data::SseHandle>>| {
        ...
        data::subscribe_lines(&url, move |line| {
            let id = counter.try_update_value(|c| { *c += 1; *c }).unwrap_or_default();
            lines.update(|v| {
                v.push((id, line));
                if v.len() > 1000 {
    ...
    let filtered_lines = Memo::new(move |_| {
        ...
        lines.get().into_iter().filter(|(_, line)| {
            ...
            if !f.is_empty() {
                let pod_match = pod.map_or(true, |p| p.to_lowercase().contains(&f));
    ...
    if trimmed.len() >= 19 {
        ...
        if trimmed.chars().nth(4) == Some('-') && 
           ...
           trimmed.chars().nth(16) == Some(':') {
            ...
            let end = trimmed[19..].find(|c: char| c == ' ' || c == 'Z' || c == '+' || c == '-')
                .map(|i| i + 19)
                .unwrap_or(19);
            ...
            if trimmed.len() > end && trimmed.chars().nth(end) == Some('.') {
                // Fractional seconds
                ts_end = trimmed[end..].find(|c: char| !c.is_ascii_digit())
                    .map(|i| i + end)
                    .unwrap_or(trimmed.len());
            }
            if trimmed.len() > ts_end && (trimmed.chars().nth(ts_end) == Some('Z') || trimmed.chars().nth(ts_end) == Some('+') || trimmed.chars().nth(ts_end) == Some('-')) {
                // Timezone
                ts_end += 1;
