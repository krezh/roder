...
pub(crate) fn pod_cells(
    ...
    let total = statuses.map(|a| a.len()).unwrap_or(0)
        + sidecars.as_ref().map(|s| s.len()).unwrap_or(0);
    let ready = statuses
        .map(|a| a.iter().filter(|c| c["ready"].as_bool() == Some(true)).count())
        .unwrap_or(0)
        + sidecars
            .as_ref()
            .map(|s| s.iter().filter(|c| c["ready"].as_bool() == Some(true)).count())
            .unwrap_or(0);
    let restarts: i64 = statuses
        .map(|a| a.iter().map(|c| c["restartCount"].as_i64().unwrap_or(0)).sum())
        .unwrap_or(0);
    ...
    let cpu_trend = usage.map_or(Trend::None, |e| if e.has_prev() { e.trend_cpu() } else { Trend::None });
    let mem_trend = usage.map_or(Trend::None, |e| if e.has_prev() { e.trend_mem() } else { Trend::None });
