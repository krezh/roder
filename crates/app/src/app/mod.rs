...
pub fn App() -> impl IntoView {
    ...
    if let Some(ns) = saved.as_ref().and_then(|v| v.get("ns").and_then(|x| x.as_str())) {
    ...
    let cat_res =
        LocalResource::new(|| async { data::fetch_json::<Vec<ResourceKind>>("/api/resources").await });
    Effect::new(move |_| {
    ...
    Effect::new(move |_| {
        let handle = window_event_listener(ev::keydown, move |e| {
            ...
            } else if e.ctrl_key() && key.eq_ignore_ascii_case("z") && !data::is_text_input_focused() {
