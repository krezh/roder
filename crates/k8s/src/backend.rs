...
pub struct Backend {
    ...
    can_cache: tokio::sync::RwLock<HashMap<(String, String, Option<String>), (std::time::Instant, bool)>>,
}
...
impl Backend {
        ...
        let mut obj = match self.registry.cached_object(key, namespace, name).await {
            ...
            None => {
                ...
                let api: Api<DynamicObject> =
                    make_api(self.client(), &entry.api_resource, entry.kind.namespaced, namespace);
    ...
    async fn events_for(
        ...
        let mut events: Vec<ObjectEvent> = list
            ...
            .map(|e| ObjectEvent {
                ...
                age: e.last_timestamp.as_ref().and_then(|t| ts_string(t)),
    ...
    async fn compute_overview(&self) -> Result<ClusterOverview, K8sError> {
        ...
        let nodes = node_list
            ...
            .map(|n| {
                ...
                let kubelet_version = info.map(|i| i.kubelet_version.clone()).filter(|s| !s.is_empty());
    ...
    fn dyn_api(&self, key: &str, ns: Option<&str>) -> Result<Api<DynamicObject>, K8sError> {
        let entry = self.entry(key)?;
        Ok(make_api(self.client(), &entry.api_resource, entry.kind.namespaced, ns))
    }
    ...
    pub async fn pod_metrics_history(
        ...
        Ok(self.registry.pod_metrics_history(namespace, name).await.unwrap_or_default())
    }
