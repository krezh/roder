...
/// Add security headers to every response (mirrors kauth's middleware). The CSP
/// allows the wasm runtime + same-origin SSE/fetch.
...
pub async fn login(State(state): State<AppState>) -> Response {
    ...
    let cookie = set_cookie(LOGIN_COOKIE, &pending_id, state.config.secure_cookies(), 600);
    (
        ...
        (Some(_), Some(code)) => {
                ...
                .await
            {
                Ok(tokens) => {
                    ...
                    (
                        [
                            (header::SET_COOKIE, set_cookie(SESSION_COOKIE, &sid, secure, 604800)),
