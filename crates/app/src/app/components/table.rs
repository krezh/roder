    ...
    F: Fn() -> String + Copy + Send + Sync + 'static,
{
    ...
    if !no_flash {
        Effect::new(move |prev: Option<String>| {
            ...
            if let Some(p) = prev {
                if p != v {
                    flash.set(true);
                    set_timeout(move || flash.set(false), std::time::Duration::from_millis(1500));
                }
            }
            v
        });
    ...
    let trend_class = move || trend.map(|t| match t.get() {
    ...
    view! {
        <div class=format!("cell {class}") class:flash=move || flash.get()
            ...
            <div class="cw"><div class="cwi">
                {move || value().replace('\n', ", ")}
                {move || trend_arrow().map(|a| view! { <span class=move || trend_class()>{a}</span> })}
            </div></div>
        </div>
    }
}
...
#[component]
pub(crate) fn ScaleControl<F>(
    run: F,
    #[prop(into)] current: Signal<Option<i32>>,
) -> impl IntoView
where
