...
pub(crate) fn RowDetail(target: DetailTarget) -> impl IntoView {
    ...
    let is_scalable = group == "apps"
        && matches!(kind.as_str(), "Deployment" | "StatefulSet" | "ReplicaSet");
    ...
    let obj = LocalResource::new(move || {
        let t = t_obj.clone();
        async move {
            data::fetch_json::<ObjectDetail>(&data::detail_url(&t.key, t.namespace.as_deref(), &t.name))
                .await
                .ok()
        }
    });
    ...
    let can_patch = move || {
        perms.get().flatten().and_then(|v| v.get("patch").and_then(|b| b.as_bool())).unwrap_or(false)
    };
    let can_delete = move || {
        perms.get().flatten().and_then(|v| v.get("delete").and_then(|b| b.as_bool())).unwrap_or(false)
    };
