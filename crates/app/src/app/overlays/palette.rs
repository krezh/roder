...
fn fuzzy_match(pattern: &str, text: &str) -> Option<FuzzyMatch> {
    ...
    if pattern_chars.is_empty() {
        return Some(FuzzyMatch { positions: vec![], score: 0 });
    }
    ...
    for (idx, &ch) in text_chars.iter().enumerate() {
        if pattern_idx < pattern_chars.len() && ch == pattern_chars[pattern_idx] {
            ...
            if idx == 0 || text_chars[idx - 1] == '_' || text_chars[idx - 1] == '-' || text_chars[idx - 1] == ' ' {
                score += 15;
            }
...
impl ParsedQuery {
    fn parse(query: &str) -> Self {
        ...
        result.text = text_parts.into_iter().collect::<Vec<_>>().join(" ").to_lowercase();
        result
    }
}
...
pub(crate) fn CommandPalette() -> impl IntoView {
    ...
    let target_kinds = Memo::new(move |_| {
        ...
        for kind_name in &p.kinds {
            ...
            if let Some(k) = kinds.iter().find(|k| fuzzy_match(kind_name, &k.plural).is_some()) {
                result.push(k.clone());
            }
        }
    ...
    let apply_search = move || {
        ...
        if p.kinds.len() == 1 {
            ...
            // Try fuzzy match on plural
            else if let Some(k) = kinds.iter().find(|k| fuzzy_match(kind_name, &k.plural).is_some()) {
                selected_kind.set(Some(k.clone()));
            }
        }
        ...
        match e.key().as_str() {
            ...
            "ArrowUp" => {
                if let Some(suggestions) = autocomplete_suggestions.get() {
                    if !suggestions.is_empty() {
                        let idx = autocomplete_index.get();
                        autocomplete_index.set(if idx == 0 { suggestions.len() - 1 } else { idx - 1 });
                        e.prevent_default();
                    }
                }
            }
                            ...
                            >
                                {
                                    ...
                                    let apply = apply_autocomplete.clone();
                                ...
                                <For each=move || resources.clone() key=|(_, r, _)| r.uid.clone() let:item>
                                    {
                                        ...
                                        let choose = choose_resource.clone();
                                        let k = kind.clone();
                                        let row = r.clone();
...
fn filter_resources(
    ...
    let mut v: Vec<(ResourceKind, ResourceRow, Vec<usize>, i32)> = rows
        ...
        .filter_map(|r| {
            ...
            let name_match = if query.text.is_empty() {
                Some(FuzzyMatch { positions: vec![], score: 0 })
            } else {
    ...
    v.sort_by(|a, b| {
        b.3.cmp(&a.3).then_with(|| a.1.name.cmp(&b.1.name))
    });
    v.truncate(100);
