...
use super::format::{endpoints_summary, hpa_targets, short_access_mode};
use super::status::{cond_to_status, condition_status, generic_reason, generic_status};
...
pub(crate) fn pvc_cells(data: &Value) -> (Vec<String>, RowStatus) {
    let phase = str_at(data, &["status", "phase"]).unwrap_or_default();
    let cap = str_at(data, &["status", "capacity", "storage"]).unwrap_or_default();
...
pub(crate) fn generic_cells(data: &Value) -> (Vec<String>, RowStatus) {
    (vec![generic_reason(data)], generic_status(data))
}
...
pub(crate) fn hpa_cells(data: &Value) -> (Vec<String>, RowStatus) {
    ...
    let reference = if kind.is_empty() { name } else { format!("{kind}/{name}") };
